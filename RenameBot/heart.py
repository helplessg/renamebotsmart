...
async def _rename(bot: Client, msg: Message):
    ...
    try:
        new_name_message = await bot.ask(
            msg.chat.id,
            "**What should be the new name ?** **[To cancel send** **`/cancel`]"**,
        ...
        surely_question = await bot.send_message(
            msg.chat.id,
            f"📝'`{new_name}`'**What should be the new name ?** **If yes, reply with 'y' If no reply with 'n' \nNew name will be asked again To cancel send** **`/cancel`**"
        ...
        await msg.reply(
            f"Error : {e} \n\nTry Again and if still doesn't work then forward this message to 📞 **ᴄᴏɴᴛᴀᴄᴛ** : **[ᴄʟɪᴄᴋ ʜᴇʀᴇ](http://t.me/GlitchAssistantBot)**")
    finally:
        q = SESSION.query(Users).get(msg.from_user.id)
        q.running = False
        SESSION.commit()
